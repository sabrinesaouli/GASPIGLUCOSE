...
int GeneticAlgorithm::fitness(Solution &solution)
{
    ...
    else
    {
        bool sat = false;
        int fitness = 0;
        Glucose::ClauseAllocator &ca = solver_.getCa();
        Glucose::vec<Glucose::CRef> &clauses = solver_.getOriginalclauses();
        ...
        for (int c = 0; c < clauses.size(); ++c)
        {
            ...
            Glucose::Clause &clause = ca[clauses[c]];

            for (int j = 0; j < clause.size(); ++j)
            {
                int cur_var = Glucose::var(clause[j]) + 1;

                if (Glucose::sign(clause[j]) == solution[cur_var])
                {
                    sat = true;
                    break;
                }
...
void SAGA::initialize_polarity(Solution &solution, Glucose::Solver &solver)
{
